...
//jQuery("#iframe").contents().find("iframe").contents().find("video")
...
(function cxtFn(setConfig){
  ...
  function goRun(setConfig){
	...
	// 关闭弹窗
	window.alert=null;
    var iframVideo = jQuery("#iframe").contents().find("iframe").contents();
    // 移除监听（可以倍数可以一步到最后）
    iframVideo.find(".x-body").unbind();
	// 切换低画质
	iframVideo.find(".vjs-resolution-button-label").trigger("click");
	console.log("切换画质OK")
    // 播放
    ...
    setTimeout(function(){
      ...
      v[0].play();
      console.log("正在播放:"+jQuery("#iframe").contents().find("p .ans-attach-ct .ans-job-icon").text());
    },1000);
    // 静音
   ...
   //延迟3秒执行调节播放速率
      setTimeout(function () {
       if(v[0].playbackRate!=speed)
        ...
        // jQuery(".speedBox").eq(0).trigger("click");
        console.log("已经开启"+speed+"倍速！");
      },2000);
    // 答题
    if(autodt=="true"&&videoDot==undefined){
      // 先判断有没有题目
      videoDot = jQuery("#iframe").contents().find("iframe").contents().find(".x-container");
    ...
    if(autonext=="true") 
      ...
      // 视频结束
      var menuList = jQuery("#coursetree .ncells");
      var nextindex =jQuery("#coursetree .ncells h4.currents").removeClass("currents").parent().index();
      menuList.eq(nextindex).find("h4").addClass("currents");
	  menuList.eq(nextindex).find("span").trigger("click");
      // 下一章
        console.log("正在播放下一章！....");
        setTimeout(function(){
      ...
      if(request&&request.form == "popup"){ 
        ...
        getSetInfo(function(setConfig){ 
          if(setConfig.state=="true") sendResponse("恭喜你,最爱助手正在运行中....");
            ...
            jQuery("#iframe").contents().find("iframe").contents().find("video")[0].playbackRate=1;
            if(timer) clearInterval(timer);
          } 
